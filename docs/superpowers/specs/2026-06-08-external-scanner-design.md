...
- **`ScannerEvents`** — app-level `MutableSharedFlow<String>` (replay 0, small extra buffer) exposing `val scans: SharedFlow<String>` and `suspend fun emit(code)` / `fun tryEmit(code)`. Owned as a singleton by `AppContainer`.
...
- **`AppContainer`** — construct/expose the singleton `ScannerEvents` alongside the existing lazy repos.
...
1. **Speed** — consecutive characters arrive under `GAP_RESET_MS` (~100 ms). A human keypress gap resets the buffer, so manual typing never accumulates into a scan.
...
Because non-scan keys return `false`, the PIN pad, search boxes, and forms keep working unchanged. The very first character of a burst (which can't yet be known to be fast) may briefly reach a focused field; this is cosmetic and rare on a counter tablet.
...
- **Scan with no relevant screen open** (e.g. on the Home dashboard) → no collector is mounted, so the event is simply dropped (replay 0). No crash, no queueing.
...
## Tuning defaults
`GAP_RESET_MS = 100`, `MIN_LEN = 3`, terminator = `BOTH` (Enter or Tab). All adjustable; terminator is user-configurable in Settings.
